...
builder.Host.UseSerilog((context, config) => config
    ...
    .Enrich.WithProperty("servico", "Carrefour.MS.FluxoDeCaixa")
    .WriteTo.Console(new JsonFormatter()));

// Application layer (MediatR + FluentValidation pipeline)
builder.Services.AddApplicationServices();

// Infrastructure — Marten (event store) + MassTransit como PUBLICADOR.
// Nenhum consumer é registrado aqui: a consolidação roda no
// Carrefour.WKR.Consolidacao (ADR-15). Registrar o consumer neste processo
// reintroduziria o acoplamento que o RNF do desafio proíbe — o registro de
// lançamentos voltaria a compartilhar destino com a consolidação.
builder.Services.AddInfrastructureServices(builder.Configuration);

// Consolidação — apenas o lado de LEITURA. A escrita do read model é do worker.
builder.Services.AddConsolidadoRepositorio(builder.Configuration);
builder.Services.AddConsolidadoConsultas();
...
{
    /// <summary>
    /// Âncora pública para os testes de integração hospedarem este serviço via
    /// <c>WebApplicationFactory&lt;PontoDeEntrada&gt;</c>.
    ///
    /// Usamos uma âncora nomeada em vez do <c>Program</c> global porque o projeto
    /// de testes referencia os dois serviços (MS e WKR) — e dois tipos
    /// <c>Program</c> visíveis no mesmo projeto dariam ambiguidade (CS0433).
    /// A WebApplicationFactory localiza o ponto de entrada pelo <i>assembly</i>
    /// do tipo informado, então qualquer tipo público deste projeto serve.
    /// </summary>
